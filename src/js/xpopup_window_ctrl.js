(function ($) {
    ...
    class XpopupWindowCtrl {
        _buildBoxElements(winData) {
            ...
            const makeSpinner = function (type) {
                ...
                const cssClass = builtinTypes.includes(type)
                    ...
                    : type;
                return '<span class="b-xpopup-loader__spinner ' + cssClass + '"></span>';
            };
            ...
            if (loaderType !== 'none') {
                ...
                if (winData.opts.loaderPlaceholder) {
                    ...
                    if ($placeholderSource.length) {
                        loaderHTML =
                            '<div class="b-xpopup-loader-placeholder">' +
                            $placeholderSource.html() +
                            '</div>';
            ...
            if (DEBUG) console.debug('_buildBoxElements:' + winData.boxId + '/ ' + winData.id);
        } //_buildBoxElements
        ...
        _removeBoxElements(winData) {
            ...
            if (DEBUG) console.debug('_removeBoxElements:' + winData.boxId + '/ ' + winData.id);

            // ОБНОВЛЕНИЕ: очищаем ResizeObserver перед удалением элементов
        ...
        _bindEventListeners(winData) {
            ...
            /// Box event listeners
            el.$box_and_bg.on('mousedown' + $.xpopup.EVENT_NS, function (e) {
            ...
            // ОБНОВЛЕНИЕ: нативные touch-события вместо jQuery-плагина свайпов
            if (winData.opts.type == 'image') {
                let pointerStartX = 0;
                let pointerStartY = 0;
                ...
                el.$container.on('pointerup' + $.xpopup.EVENT_NS, function (e) {
                    ...
                    if (absDeltaX > absDeltaY * swipeRatioThreshold && absDeltaX > swipeThreshold) {
                        const noFocus = $(':focus').filter('textarea,input,select').size() == 0;
                        if (!noFocus) {
                            return;
                        }
            ...
            // ОБНОВЛЕНИЕ: zoom по двойному клику на изображении
            if (winData.opts.type == 'image') {
                el.$container.on('click' + $.xpopup.EVENT_NS, '.b-xpopup-image__img', function (e) {
                    e.preventDefault();
                    const $box = el.$box;
            ...
            // ОБНОВЛЕНИЕ: сохраняем ссылку на ResizeObserver для последующей очистки при закрытии
            if (el._resizeObserver) {
                el._resizeObserver.disconnect();
            }

            el._resizeObserver = new ResizeObserver((entries) => {
                // ОБНОВЛЕНИЕ: проверяем, изменился ли реально размер
                const newWidth = el.$contentContainer.outerWidth();
                const newHeight = el.$contentContainer.outerHeight();

                if (el._lastContentWidth === newWidth && el._lastContentHeight === newHeight) {
        ...
        _buildWindowData(options, triggerEl) {
            ...
            data.pluginDefaults =
                $.xpopup.pluginGet('content_type.' + data.autoDetectedOptions.type).defaults || {};
            ...
            if (data.opts.id) {
                ...
                let nValuesChars = 0;
                let nKeysChars = 0;
                // ОБНОВЛЕНИЕ: замена $.each на Object.keys().forEach()
                Object.keys(data.initial).forEach(function (key) {
                    nArgs++;
                    nKeysChars += key.toString().length;
                    nValuesChars += data.initial[key] ? data.initial[key].toString().length : 0;
                });
                data.id =
                    '' +
                    data.opts.type +
            ...
            if (galleryId) {
                ...
                // ОБНОВЛЕНИЕ: замена $.each на нативный each (jQuery object)
                ...
                // ОБНОВЛЕНИЕ: пробрасываем опцию galleryLoop в данные окна
                data.galleryLoop =
        ...
        _buildWindowResultOptions(windowData) {
            ...
            // if (o.animation == true) {
            //     if (xpopupUtils.isPhone() || !o.animationDelay) {
            ...
            if (typeof o.headerActions !== 'undefined' && typeof o.headerActions === 'string') {
            ...
            //Данные для подвала
            if (typeof o.footerActions !== 'undefined' && typeof o.footerActions === 'string') {
            ...
            // ОБНОВЛЕНИЕ: замена $.isArray на Array.isArray
            const skins = Array.isArray(o.skin) ? o.skin : o.skin.split('.');
            o.skinClasses = '';
            // ОБНОВЛЕНИЕ: замена $.each на нативный forEach
            ...
            if (o.skinMod) {
                ...
                //    skinMod: ['no-borders','centered']
                // ОБНОВЛЕНИЕ: замена $.isArray на Array.isArray
                const skinMods = Array.isArray(o.skinMod) ? o.skinMod : o.skinMod.split('.');
                o.skinModClasses = '';
                // ОБНОВЛЕНИЕ: замена $.each на нативный forEach
        ...
        fetchContent() {
            ...
            // ===================================================================
            // minLoadingTimeout — эмуляция загрузки
            // ===================================================================
            // newWinData.opts.minLoadingTimeout = 0;
            if (newWinData.opts.minLoadingTimeout) {
                newWinData._loadingStartedAt = Date.now();
                newWinData._loadingDeferred = true;
                ...
                if (newWinData.opts.html) {
                    $.xpopup.callDelayed(
                        'renderContent',
                        () => win.renderContent(),
                        newWinData.opts.minLoadingTimeout,
                    );
                } else {
                    const plugin = $.xpopup.pluginGetContentTypePlugin(newWinData.opts.type);
        ...
        renderContent() {
            ...
            // ===================================================================
            // minLoadingTimeout — отложенный рендер
            // Если контент загрузился быстрее минимального времени (например, AJAX-ответ
            // пришёл раньше), откладываем фактический рендер до истечения таймаута.
            // В это время пользователь видит лоадер или placeholder.
            // ===================================================================
            if (newWinData._loadingDeferred && newWinData._loadingStartedAt) {
                const elapsed = Date.now() - newWinData._loadingStartedAt;
                const remaining = newWinData.opts.minLoadingTimeout - elapsed;

                if (remaining > 0) {
                    // Контент готов, но нужно подождать ещё remaining мс
                    // Кэшируем winData (он может измениться) и откладываем рендер
                    const cachedWinDataId = newWinData.id;
                    setTimeout(function () {
                        const st = win.getStorage();
                        const deferredWinData = st.getBoxWindow(cachedWinDataId);
                        if (deferredWinData) {
                            // Сбрасываем флаг, чтобы избежать повторной задержки
                            delete deferredWinData._loadingDeferred;
                            delete deferredWinData._loadingStartedAt;

                            // Вызываем рендер для этого окна
                            win._performRender(deferredWinData);
                        }
                    }, remaining);
                    return;
                } else {
                    // Таймаут уже истёк — сбрасываем флаг и рендерим сразу
                    delete newWinData._loadingDeferred;
                    delete newWinData._loadingStartedAt;
                }
            }

            // Обычный рендер
            win._performRender(newWinData);
        }

        /**
         * Выполняет фактический рендер контента (вставка в DOM, обновление шапки, подвала и т.д.).
         * Выделен в отдельный метод, чтобы можно было вызывать отложенно (для minLoadingTimeout).
         *
         * @param {object} winData - данные окна для рендера
         */
        _performRender(winData) {
            const win = this;
            const st = this.getStorage();
            const el = st.getElements();
            ...
            if (typeof winData.opts.boxId !== 'undefined' && winData.opts.boxId != boxData.id) {
            ...
            // Вставка контента
            win.setContent(winData.opts.html, winData.opts.type);
            win._updateBoxSize();
            ...
            if (boxData.status != 'opened') {
                if (DEBUG) console.debug('_performRender: boxData.status =', boxData.status);
         ...
         *
         * @param {string|jQuery} newContent - новый HTML-контент для отображения
         * @param {string} type - тип контента ('inline', 'ajax', 'image', 'iframe')
         */
        setContent(newContent, type) {
            ...
            if (el.$imageActions) {
                ...
                if (shareAction === 'always') {
                    ...
                    showShare = winData.opts.share === true || winData.opts.share === 'true';
                }

                if (showShare) {
                    actionsHtml +=
                        '<a href="javascript:void(0);" class="b-xpopup__image-action b-xpopup__image-action_share b-xpopup-option_close_off" title="Поделиться"></a>';
                }
                ...
                if (showDownload && downloadUrl) {
                    actionsHtml +=
                        '<a href="' +
                        downloadUrl +
                        '" target="_blank" download class="b-xpopup__image-action b-xpopup__image-action_download b-xpopup-option_close_off" title="Скачать"></a>';
                }

                el.$imageActions.html(actionsHtml);

                const $shareBtn = el.$imageActions.find('.b-xpopup__image-action_share');
            ...
            // ===================================================================
            if (winData.parentId) {
                el.$box_and_bg.addClass('b-xpopup_has-parent-id');
                if (winData.opts.backBtn) {
                    el.$box_and_bg.addClass('b-xpopup_has-back');
                } else {
                    el.$box_and_bg.removeClass('b-xpopup_has-back');
                }
            } else {
                el.$box_and_bg.removeClass('b-xpopup_has-parent-id');
                el.$box_and_bg.removeClass('b-xpopup_has-back');
            }
            ...
            if ($boxAnimTarget.length) {
                $boxAnimTarget
                    .removeClass('b-xpopup--box-animation_false b-xpopup--box-animation_true')
                    .addClass(
                        winData.opts.boxAnimation == false
                            ? 'b-xpopup--box-animation_false'
                            : 'b-xpopup--box-animation_true',
        ...
        /**
         * ОБНОВЛЕНИЕ: добавлен параметр errorMessage для передачи текста ошибки
         */
        setWindowStatus(newStatus, errorMessage) {
            ...
            // ОБНОВЛЕНИЕ: сохраняем сообщение об ошибке в данных бокса для возможного использования
         ...
         */
        close() {
            if (DEBUG) console.debug('close called, stack:', new Error().stack);
         ...
         */
        _close() {
            if (DEBUG) console.debug('XpopupWidowCtrl._close called');
            ...
            // ===================================================================
            // Восстановление элемента-заглушки (loaderPlaceholder)
            // Если при открытии использовался placeholder, его исходный элемент
            // был скрыт через .hide(). При закрытии нужно показать его снова.
            // ===================================================================
            // if (winData._placeholderUsed && winData.opts.loaderPlaceholder) {
            //     const $sourcePlaceholder = $(winData.opts.loaderPlaceholder);
            //     if ($sourcePlaceholder.length && $sourcePlaceholder.is(':hidden')) {
            //         $sourcePlaceholder.show();
            //     }
            //     winData._placeholderUsed = false;
            // }

            // Запускаем анимации и получаем промисы
            // debugger
            ...
            const finishClosing = function () {
                // Дополнительная страховка восстановления элемента-заглушки
                // на случай, если _close был вызван без предварительного восстановления
                // if (winData._placeholderUsed && winData.opts.loaderPlaceholder) {
                //     const $sourcePlaceholder = $(winData.opts.loaderPlaceholder);
                //     if ($sourcePlaceholder.length && $sourcePlaceholder.is(':hidden')) {
                //         $sourcePlaceholder.show();
...
debugger
                ...
                if (closePopup) {
                    ...
                    $document.off('keyup' + $.xpopup.EVENT_NS + ' focusin' + $.xpopup.EVENT_NS);
                    $document.add($window).off($.xpopup.EVENT_NS);
        ...
        //
        // ОБНОВЛЕНИЕ: оптимизация - замена $target.parents('body').size() != 0 на document.body.contains(target)
        ...
        _updateBoxAnimation(animationType) {
            ...
            const timing = timingMap[effectName] || { duration: 300, easing: 'ease', fill: 'both' };
            ...
            if (winData.opts.headerCaption && !winData.opts.headerHide) {
                ...
                if (caption) {
                    if (
                        ...
                        caption =
                            "<span class='b-xtip' title='" + caption + "'>" + caption + '</span>';
                    }
                }
                ...
                if (type != 'image') {
                    ...
                    if (winData.opts.backBtn && winData.parentId) {
                        headerBack =
                            "<span title='Назад' class='b-xpopup__back b-xpopup-action b-xpopup-action_type_back'><span class='b-xpopup-ico b-xpopup-ico_back'></span></span>";
                    ...
                    if (
                        ...
                        headerActions +=
                            "<div class='b-xpopup__header-actions-toggler' role='button'><span class='b-xpopup-ico b-xpopup-ico_actions'></span><span class='b-xpopup__header-actions-toggler-text'>Действия</span></div>";
                    ...
                    if (!calcMode) {
                        $header.find('.b-xpopup__header-actions-toggler').on('click', function () {
        ...
        _updateArrow(type) {
            ...
            // ОБНОВЛЕНИЕ: поддержка карусели (galleryLoop) для галереи
            else if (type == 'left') {
                const galleryItemsCount = winData.galleryItemsCount;
            ...
            if (arrowData || $arrowTrigger) {
                ...
                const arrowClasses =
                    'b-xpopup__arrow b-xpopup__arrow_' + type + '  b-xpopup-option_close_off';
                $arrow = $(
                    "<span class='" +
                        arrowClasses +
                        "' ><span class='b-xpopup__arrow-icon'></span></span>",
                );
                ...
                $arrow.click(function () {
                    ...
                    inheritOpts.forEach(function (key) {
                        if (winData.opts[key] !== undefined && arrowData[key] === undefined) {
        ...
        _updateContainerSizes() {
            ...
            const vertMargins =
                parseInt(el.$box.css('padding-top')) + parseInt(el.$box.css('padding-bottom'));
            ...
            if (winData.opts.sizeMode == 'auto') {
                ...
                winData._isFixedSize = sizeModeFixed || (hasExplicitHeight && hasExplicitWidth);

                // Нужно ли вычислять размеры контента через клонирование DOM
                const needCalcSize = !sizeModeFixed && !css.maxWidth && !hasExplicitWidth;
                ...
                if (needCalcSize) {
                    ...
                    if (!contentSize) {
                        ...
                        const hasContentSize = contentSize && Object.keys(contentSize).length > 0;
            ...
            // ===================================================================
            if (el.$closeBtn && el.$closeBtn.hasClass('b-xpopup__close_type_outside')) {
                const MARGIN = 15;
                ...
                el.$closeBtn[0].style.setProperty('--xpopup__close_outside_top', posY + 'px');
                el.$closeBtn[0].style.setProperty('--xpopup__close_outside_left', posX + 'px');
            }
        }
        ...
        _calcContentSize(css) {
            ...
            // ОБНОВЛЕНИЕ: замена $.isEmptyObject на проверку количества ключей
            if (Object.keys(css).length > 0) {
                $container.css(css);
            }
     ...
     *
     * ОБНОВЛЕНИЕ: замена $.isNumeric на проверку typeof + isNaN
     */
