class XpopupStorage {
    ...
    deleteBox() {
        if ($.xpopup.DEBUG) console.debug('XpopupStorage.deleteBox called');
    ...
    findBoxIndex(id) {
        let matchedIndex = -1;
        // ОБНОВЛЕНИЕ: замена $.each на нативный forEach с досрочным выходом через some
    ...
    countUsableBoxes(excludeBoxId) {
        let n = 0;
        // ОБНОВЛЕНИЕ: замена $.each на нативный перебор массива
        ...
        for (let i = 0; i < boxes.length; i++) {
            ...
            if (box.status != 'closed' && box.status != 'opening' && box.windows && box.windows.length > 0) {
                n++;
            }
        }
        return n;
    ...
    getWindowIndex(id, boxId) {
        ...
        // ОБНОВЛЕНИЕ: замена $.each на нативный some для возможности досрочного выхода
    ...
    findWindows(filters, boxId) {
        ...
        box.windows.forEach(function (item, index) {
            ...
            // В оригинале был $.each с возможностью прерывания через return false,
            // но здесь фильтры не предполагают прерывания, поэтому forEach подходит
        });
